...
and why every return value is json.dumps()'d rather than a raw list/dict
(prevents empty-list tool-message content errors on providers like Groq).
"""
import json
...
def build_task_tools(service):
    ...
    def list_tasks(show_completed: bool = False, max_results: int = 50):
        ...
        result = tasks_service_module.list_tasks(service=service, show_completed=show_completed, max_results=max_results)
    ...
    def create_task(title: str, notes: str | None = None, due: str | None = None):
        ...
        result = tasks_service_module.create_task(service=service, title=title, notes=notes, due=due)
        return json.dumps(result)
    ...
    def update_task(task_id: str, updates: dict):
        ...
        """
        result = tasks_service_module.update_task(service=service, task_id=task_id, updates=updates)
        return json.dumps(result)
